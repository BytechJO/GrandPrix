import React, { useState } from "react";
import ValidationAlert from "../../Popup/ValidationAlert";
import ScoreCardEnhanced from "../../Popup/ScoreCard";
import "./WB_Unit4_Page24_Q7.css";
/* 🔴 استيراد الصور */
import img1 from "../../../assets/workpages/SVG25/1.svg";
import img2 from "../../../assets/workpages/SVG25/2.svg";
import img3 from "../../../assets/workpages/SVG25/3.svg";
import img4 from "../../../assets/workpages/SVG25/4.svg";
import img5 from "../../../assets/workpages/SVG25/5.svg";

/* 🔴 الإجابات النصية */
const correctAnswers = {
  a: "pied",
  b: "avion",
  c: "métro",
  d: "voiture",
 
};

/* 🔴 التطابق (الحرف → رقم الصورة) */
const correctMatches = {
  a: "5",
  b: "3",
  c: "4",
  d: "2",
 
};



/* 🔴 الصور للـ popup */
const imagesPopup = [
  { id: "a", src: img1 },
  { id: "b", src: img2 },
  { id: "c", src: img3 },
  { id: "d", src: img4 },
  { id: "e", src: img5 },
];

const Page5_Q1_CleanAudio = () => {
  const [inputs, setInputs] = useState({}); // للإجابات النصية
  const [matches, setMatches] = useState({}); // للتطابقات
  const [score, setScore] = useState(null);
  const [showImagesPopup, setShowImagesPopup] = useState(false);

  // تحديث الإجابات النصية
  const handleInputChange = (questionId, value) => {
    setInputs({ ...inputs, [questionId]: value });
  };

  // تحديث التطابقات
  const handleMatchChange = (letter, value) => {
    setMatches({ ...matches, [letter]: value.trim() });
  };

  // التحقق من الإجابات
  const checkAnswer = () => {
    let correctCount = 0;

    // إجابات النصية
    Object.keys(correctAnswers).forEach((questionId) => {
      const userAnswer = inputs[questionId]
        ? inputs[questionId].toLowerCase().trim()
        : "";
      const correctAnswer = correctAnswers[questionId].toLowerCase().trim();
      if (userAnswer === correctAnswer) correctCount++;
    });

    // التطابقات
    Object.keys(correctMatches).forEach((letter) => {
      const userMatch = matches[letter] ? matches[letter] : "";
      const correctMatch = correctMatches[letter];
      if (userMatch === correctMatch) correctCount++;
    });

    const totalQuestions =
      Object.keys(correctAnswers).length + Object.keys(correctMatches).length;
    setScore({ correct: correctCount, total: totalQuestions });

    if (correctCount === 0) {
      ValidationAlert.info(
        `Toutes les réponses sont incorrectes (${correctCount}/${totalQuestions})`,
        "Essayez encore!"
      );
    } else if (correctCount === totalQuestions) {
      ValidationAlert.success(
        `Excellent! (${correctCount}/${totalQuestions})`,
        "Toutes les réponses sont correctes!"
      );
    } else {
      ValidationAlert.error(
        `Vous avez ${correctCount} sur ${totalQuestions} corrects.`,
        "Presque!"
      );
    }
  };

  // عرض الإجابات الصحيحة
  const showAnswerFunc = () => {
    setInputs(correctAnswers);
    setMatches(correctMatches);
  };

  // إعادة التمرين
  const resetExercise = () => {
    setInputs({});
    setMatches({});
    setScore(null);
  };

  const openImagesPopup = () => setShowImagesPopup(true);
  const closeImagesPopup = () => setShowImagesPopup(false);

  return (
    <div className="page24-wrapper1 flex flex-col items-center justify-center p-4">
      <header
        className="header-title-page1 w-full text-left mb-4"
        style={{
          marginLeft: "42%",
          color: "black",
          marginTop: "5%",
          fontSize: "25px",
          fontWeight: "bold",
        }}
      >
        <span
          style={{ backgroundColor: "#ce5b66" }}
          className="ex-A"
        >
          4
        </span>{" "}
        <span style={{ color: "black" }} className="number-of-q">
          8
        </span>
        Déchiffre les mots et trouve le bon dessin.
      </header>

      {score && <ScoreCardEnhanced score={score} />}

      <div className="page24-main-section w-full max-w-5xl flex flex-row gap-8 mb-8">
        {/* الأسئلة */}
        <div className="page24-questions-container flex-1">
          <div className="page24-questions-title text-xl font-bold mb-4 text-center">
            Complétez les phrases
          </div>
          <div className="page24-questions-list space-y-4">
            {Object.entries(correctAnswers).map(([questionId]) => {
              const texts = {
                a: "e d p i ______ ",
                b: "o v n i a ______ ",
                c: "r o t m é ______ ",
                d: "t r o u v e i______ ",
             
              };
              const sentenceParts = texts[questionId].split("______");
              return (
                <div
                  key={questionId}
                  className="page24-question-item flex items-center gap-3"
                >
                  <span className="page24-question-letter font-bold text-lg text-red-600 min-w-6">
                    {questionId}
                  </span>
                  <span className="page24-question-text text-lg">
                    {sentenceParts[0]}
                    <input
                      type="text"
                      className="page24-question-input mx-2 border-b-2 focus:outline-none text-center"
                      style={{
                        width: "90px",
                        borderColor: inputs[questionId]
                          ? inputs[questionId].toLowerCase().trim() ===
                            correctAnswers[questionId].toLowerCase().trim()
                            ? "green"
                            : "red"
                          : "#3b82f6",
                        backgroundColor: inputs[questionId]
                          ? inputs[questionId].toLowerCase().trim() ===
                            correctAnswers[questionId].toLowerCase().trim()
                            ? "#d4f5d4"
                            : "#f5d4d4"
                          : "#f9fafb",
                      }}
                      value={inputs[questionId] || ""}
                      onChange={(e) =>
                        handleInputChange(questionId, e.target.value)
                      }
                      placeholder="..."
                    />
                    {sentenceParts[1]}
                  </span>
                </div>
              );
            })}
          </div>

          <div className="page24-show-images-btn-container mt-6 text-center">
            <button
              onClick={openImagesPopup}
              className="page24-show-images-btn px-6 py-3 bg-blue-500 text-white rounded-lg font-bold hover:bg-blue-600 transition-colors"
            >
              👁️ Voir les dessins
            </button>
          </div>
        </div>

        {/* جدول التطابق */}
        <div className="page24-match-container w-72">
          <div className="page24-match-title text-xl font-bold mb-4 text-center">
            Associez les dessins
          </div>
          <div className="page24-match-table bg-white rounded-lg shadow-lg p-4 border border-gray-200">
            <div className="page24-match-header flex justify-between font-bold border-b-2 border-gray-300 pb-2 mb-3">
              <span className="page24-match-col1 text-center w-1/2">Lettre</span>
              <span className="page24-match-col2 text-center w-1/2">Dessin</span>
            </div>
            <div className="page24-match-body space-y-3">
              {Object.entries(correctMatches).map(([letter, imgNumber]) => (
                <div
                  key={letter}
                  className="page24-match-row flex justify-between items-center"
                >
                  <span className="page24-match-letter font-bold text-lg text-center w-1/2 text-blue-600">
                    {letter.toUpperCase()}
                  </span>
                  <input
                    type="text"
                    className="page24-match-input border-2 rounded px-3 py-1 text-center text-lg font-bold uppercase w-16"
                    style={{
                      borderColor: matches[letter]
                        ? matches[letter] === imgNumber
                          ? "green"
                          : "red"
                        : "#d1d5db",
                      backgroundColor: matches[letter]
                        ? matches[letter] === imgNumber
                          ? "#d4f5d4"
                          : "#f5d4d4"
                        : "#f9fafb",
                    }}
                    value={matches[letter] || ""}
                    onChange={(e) => handleMatchChange(letter, e.target.value)}
                    placeholder="? "
                    maxLength="1"
                  />
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>

      {/* Popup الصور */}
      {showImagesPopup && (
        <div className="page24-images-popup-overlay fixed inset-0 bg-opacity-70 flex items-center justify-center z-50">
          <div className="page24-images-popup-container bg-white rounded-xl shadow-2xl w-11/13 max-w-4xl max-h-[60vh] overflow-y-auto">
            <div className="page24-popup-header flex justify-between items-center p-4 border-b border-gray-200">
              <h2 className="page24-popup-title text-2xl font-bold text-gray-800">
                Les Dessins - Trouvez la lettre correspondante
              </h2>
              <button
                onClick={closeImagesPopup}
                className="page24-close-popup-btn text-gray-500 hover:text-gray-700 text-2xl font-bold p-2"
              >
                ✕
              </button>
            </div>

            <div className="page24-popup-content p-6 grid grid-cols-3 gap-6">
              {imagesPopup.map((image) => (
                <div key={image.id} className="page24-popup-image-item flex flex-col items-center">
                  <img
                    src={image.src}
                    alt={`Dessin ${image.id.toUpperCase()}`}
                    className="page24-popup-image"
                    style={{ width: "100%", height: "auto" }}
                  />
                 
                </div>
              ))}
            </div>
          </div>
        </div>
      )}

      {/* Buttons */}
      <div className="action-buttons-container flex gap-4 mt-6">
        <button onClick={resetExercise} className="try-again-button px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">
          Recommencer ↻
        </button>
        <button onClick={showAnswerFunc} className="show-answer-btn px-4 py-2 bg-yellow-400 rounded hover:bg-yellow-500">
          Afficher la réponse
        </button>
        <button onClick={checkAnswer} className="check-button2 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
          Vérifier la réponse ✓
        </button>
      </div>
    </div>
  );
};

export default Page5_Q1_CleanAudio;
